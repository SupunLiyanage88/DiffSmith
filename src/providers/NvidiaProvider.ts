...
export class NvidiaProvider implements AIProvider {
  ...
  async configure(context: vscode.ExtensionContext): Promise<void> {
    ...
    const key = await vscode.window.showInputBox({
      title: 'CommitLoom: NVIDIA API Key',
    ...
    if (key.trim().length === 0) {
      void vscode.window.showWarningMessage('CommitLoom: empty key — NVIDIA provider is not configured.');
      return;
    }
    ...
    if (picked.isCustom) {
      const input = await vscode.window.showInputBox({
        title: 'CommitLoom: Custom NVIDIA Model',
    ...
    void vscode.window.showInformationMessage(
      `CommitLoom: NVIDIA provider configured (${getNvidiaModelLabel(next ?? currentModel)}).`
    );
  }
  ...
  async generateCommitMessage(diff: string, options: GenerateOptions): Promise<string> {
    ...
    try {
      ...
      if (res.status === 401) {
        const err = new Error('Invalid NVIDIA API key (401). Check your key via "CommitLoom: Configure Provider".');
