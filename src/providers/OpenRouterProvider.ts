...
const SECRET_KEY = 'commitloom.openrouter.apiKey';
const APP_URL = 'https://github.com/SupunLiyanage88/CommitForge';
const APP_TITLE = 'CommitLoom';
...
export class OpenRouterProvider implements AIProvider {
  ...
  async configure(context: vscode.ExtensionContext): Promise<void> {
    ...
    const key = await vscode.window.showInputBox({
      title: 'CommitLoom: OpenRouter API Key',
    ...
    if (key.trim().length === 0) {
      void vscode.window.showWarningMessage('CommitLoom: empty key — OpenRouter provider is not configured.');
      return;
    }
    ...
    const picked = await vscode.window.showQuickPick(picks, {
      title: 'CommitLoom: OpenRouter Model',
    ...
    if (picked.isCustom) {
      const input = await vscode.window.showInputBox({
        title: 'CommitLoom: Custom OpenRouter Model',
      ...
      next = input.trim();
      if (next.length === 0) {
        void vscode.window.showWarningMessage('CommitLoom: empty model — keeping the current one.');
        return;
      }
    } else {
      next = picked.modelId;
    ...
    void vscode.window.showInformationMessage(
      `CommitLoom: OpenRouter provider configured (${getOpenRouterModelLabel(next ?? currentModel)}).`
    );
  }
  ...
  async generateCommitMessage(diff: string, options: GenerateOptions): Promise<string> {
    ...
    if (!apiKey) {
      throw apiError(
        'OpenRouter API key is not set. Run "CommitLoom: Configure Provider" to set it.',
        'NOT_CONFIGURED'
      );
    }
    ...
    try {
      ...
      if (res.status === 401) {
        throw apiError(
          'Invalid OpenRouter API key (401). Check your key via "CommitLoom: Configure Provider".',
          'UNAUTHORIZED'
        );
      }
      if (res.status === 402) {
