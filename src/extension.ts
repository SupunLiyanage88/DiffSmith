...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  const output = vscode.window.createOutputChannel('CommitLoom');
  ...
  if (!(await hasGitRepo()) && !repoWarningShown) {
    ...
    void vscode.window.showInformationMessage(
      'CommitLoom: no git repository detected in the open workspace. Open a git repo to generate commit messages.'
    );
  }
}
