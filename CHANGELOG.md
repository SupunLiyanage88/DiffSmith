...
All notable changes to Diffly will be documented here.
...
- `Diffly: Generate Commit Message` (palette + Source Control button) with progress, cancellation, and Retry on failure; writes to SCM input box, never auto-commits.
- `Diffly: Configure Provider` with `SecretStorage` API key storage and model override.
- `diffly.*` settings schema + centralized `Settings.ts` loader.
