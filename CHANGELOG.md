...
All notable changes to DiffSmith will be documented here.
...
- `DiffSmith: Generate Commit Message` (palette + Source Control button) with progress, cancellation, and Retry on failure; writes to SCM input box, never auto-commits.
- `DiffSmith: Configure Provider` with `SecretStorage` API key storage and model override.
- `diffsmith.*` settings schema + centralized `Settings.ts` loader.
